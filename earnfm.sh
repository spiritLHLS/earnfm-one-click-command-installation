#!/bin/bash
#FROM https://github.com/spiritLHLS/earnfm-one-click-command-installation
...
# 输入 earnfm 的个人信息
input_token(){
  [ -z $API_Key ] && reading " Enter your API_Key, if you do not find it, open https://earn.fm/ref/JANEAG9H : " API_Key 
}
...
container_build(){
  ...
  # 删除旧容器（如有）
  docker ps -a | awk '{print $NF}' | grep -qw "$NAME" && yellow " Remove the old earnfm container.\n " && docker rm -f "$NAME" 
...
uninstall(){
  ...
  docker rmi -f $(docker images | grep earnfm/earnfm-client:latest | awk '{print $3}')
